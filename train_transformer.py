...
import math # ✅ NEW: For Positional Encoding
...
# ✅ NEW: Transformer Classifier Model
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, nhead, num_encoder_layers, dropout=0.1):
        ...
        # Transformer Encoder
        encoder_layers = nn.TransformerEncoderLayer(d_model=self.d_model, nhead=nhead, dropout=dropout, batch_first=True)
    ...
    def forward(self, x):
        ...
        x = self.input_proj(x) # Project to [batch_size, seq_len, d_model]
        x = self.pos_encoder(x)
        output = self.transformer_encoder(x) # shape: [batch_size, seq_len, d_model]
        
        # Aggregate sequence info - mean pooling is a common choice
...
def print_and_save_confusion_matrix(y_true, y_pred, bal_acc, file_path):
...
def train(args):
    ...
    # ✅ NEW: Add Transformer model option
        ...
        raise ValueError("Model must be 'lstm', 'mlp', or 'transformer'")
    ...
    for epoch in range(args.epochs):
        ...
        print(f"\nEpoch {epoch+1}: Train Loss={train_loss/len(train_loader):.4f}, Train Balanced Acc={train_bal_acc:.4f} | "
              f"Val Loss={val_loss/len(val_loader):.4f}, Val Acc={val_acc:.4f}, Balanced Val Acc={val_bal_acc:.4f}\n")
    ...
    print_and_save_confusion_matrix(all_labels, all_preds, bal_acc, cm_path)
...
if __name__ == "__main__":
    ...
    # ✅ NEW: Add 'transformer' to choices
    parser.add_argument("--model", type=str, default="lstm", choices=["lstm", "mlp", "transformer"])
    ...
    parser.add_argument("--hidden_dim", type=int, default=256, help="Hidden dimension for LSTM and Transformer")
    
    # LSTM specific
    parser.add_argument("--num_layers", type=int, default=3, help="Number of layers for LSTM")
    ...
    parser.add_argument("--num_encoder_layers", type=int, default=3, help="Number of encoder layers for Transformer")
